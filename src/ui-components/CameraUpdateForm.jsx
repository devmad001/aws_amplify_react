...
export default function CameraUpdateForm(props) {
  const {
    id: idProp,
    camera: cameraModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = cameraRecord
      ? { ...initialValues, ...cameraRecord }
      : initialValues;
    setName(cleanValues.name);
    setLatitude(cleanValues.latitude);
    setLongitude(cleanValues.longitude);
    setRtsp_stream(cleanValues.rtsp_stream);
    setRtsp_params(cleanValues.rtsp_params);
    setErrors({});
  };
  const [cameraRecord, setCameraRecord] = React.useState(cameraModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Camera, idProp)
        : cameraModelProp;
      setCameraRecord(record);
    };
    queryData();
  }, [idProp, cameraModelProp]);
  React.useEffect(resetStateValues, [cameraRecord]);
  const validations = {
    name: [{ type: "Required" }],
    latitude: [],
    longitude: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Camera.copyOf(cameraRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "CameraUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || cameraModelProp)}
